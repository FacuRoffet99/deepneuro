...
def filtPowSpetra(signal, f_low, f_high, TR, k):
    ...
    ts_filt_narrow = BandPassFilter(signal, f_low, f_high, TR, k)
...
def filtPowSpetraMultipleSubjects(signal, f_low, f_high, TR, k):
    if signal.ndim == 2:
        nSubjects = 1
        nNodes, Tmax = signal.shape  # Here we are assuming we receive only ONE subject...
        Power_Areas_filt_narrow_unsmoothed = filtPowSpetra(signal, f_low, f_high, TR, k)
    else:
        ...
        for s in range(nSubjects):
            ...
            PowSpect_filt_narrow[s] = filtPowSpetra(signal[s,:,:], f_low, f_high, TR, k).T
...
def get_omegas(time_series, f_low, f_high, TR, k):
    return 2*np.pi * np.array([filtPowSpetraMultipleSubjects(ts, f_low, f_high, TR, k) for ts in time_series])
